...
int main(void) {
    char original[]="\n"    //si se cambia original[] por *original entonces abajo en vez de usar sizeof(original) -1 se usa
    ...
    copy = string_clone(original, sizeof(original)-1);   //aca se deberia usar strlen de la libreria string.h
    printf("Original: %s\n", original);
    printf("Copia   : %s\n", copy);
    free(copy);
